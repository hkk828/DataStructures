class LinkedList:
	'''
	LinkedList is implemented with python dictionary.
	Because the python dictionary uses hashtable concept,
	LinkedList append, pop, and __getitem__ can be done in constant time. 
	'''
	...
	def pop(self, idx=-1):
		# When a LinkedList is empty, then raise an Error
		if self.size == 0:
	...
	def __getitem__(self, idx):
		...
		if idx < 0:
			return self.list[idx + self.size]
		return self.list[idx]

	def __repr__(self):
		# Represent LinkedList as "[ ]" when empty, and "a-> b-> c-> d" when it contains a, b, c, and d in order.
		if self.size == 0:
			return "[ ]"
	...
	def extend(self, other):
		# Extend self with putting other LinkedList at the end
...
if __name__ == '__main__':
	...
	pyList = [1, 2]
	newList = LinkedList()
	newList.toLinkedList(pyList)
	print(newList) # "1-> 2"

	print(newList.extend(myList)) # "1-> ... -> 8-> 8"
	
	otherList = LinkedList()
	otherList.toLinkedList([1, 2])
